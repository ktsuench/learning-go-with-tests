...
func TestMinutesInRadians(t *testing.T) {
	cases := []struct {
		...
		{simpleTime(0, 24, 18), 0.8*math.Pi + (0.6*math.Pi)/60},
	}

	for _, test := range cases {
		t.Run(testName(test.time), func(t *testing.T) {
			got := minutesInRadians(test.time)
			if !roughlyEqualFloat(got, test.angle) {
				t.Fatalf("got %v, want %v rad", got, test.angle)
			}
		})
	}
...
func TestHourHandPoint(t *testing.T) {
	angleAt8Hour := 8*(math.Pi/12) + 13*(math.Pi/720) + 57*(math.Pi/43200)
	cases := []struct {
		time  time.Time
		point Point
	}{
